...
import { DepositMoneyRequestDTO } from '../dto/deposit-money-request.dto';
...
export class AccountService {
    ...
    async createNewAccount(record: CreateAccountRequestDTO):Promise<CreateAccountReposnseDto> {
        ...
        return new Promise(async (resolve) => {
            (await this.account_repository.create(account))
                .pipe(take(1))
     ...
     */
    async getAccount(
        query: GetUserAccountRequestDTO
    ): Promise<GetUserAccountResponseDTO> {
        return new Promise(async (resolve) => {
            (await this.account_repository.get(query.id))
                .pipe(take(1))
                .subscribe((data: any) => {
                    resolve(data);
                });
    ...
    async updateAccount(
        ...
        return new Promise(async (resolve) => {
            (await this.account_repository.get(record.id))
                .pipe(take(1))
                .subscribe(async (data: any) => {
                    if (data) {
    ...
    async depositModeny(id: string, amount: DepositMoneyRequestDTO):Promise<GetUserAccountResponseDTO> {
        ...
        try {
            return new Promise(async (resolve) => {
                (await this.account_repository.get(id))
                    .pipe(take(1))
                    .subscribe(async (data: any) => {
                        (await data).balance += amount.amount;
                        (await data).balance = naiveRound(
                            (await data).balance,
                            3
                        );
                        await this.account_repository.update(data);
                    ...
                    (await this.account_repository.get(id))
                        ...
                        .subscribe(async (data: any) => {
                            if (data.balance < amount.amount) {
                                rejects(
                                    `Insufficient Funds.current balance is: ${data.balance}`
                                );
                            } else {
                                (await data).balance -= amount.amount;
                                (await data).balance = naiveRound(
